 ...
 *
 * <p>A real {@link BundleClient} wrapped in {@link EventoServerV2Adapter}
 * fires an {@link PerformanceInvocationsMessage} via
 * {@link EventoServerV2Adapter#send(java.io.Serializable)}. The server side
 * surfaces it as a {@link BusEvent.AdminNotification} that subscribers can
 * pattern-match — proving the wire convention agreed by
 * {@code EventoServerV2Adapter}, {@code BusLifecycle.onNotification}, and the
...
class AdminNotificationFlowIT {
    ...
    void eventoServerV2AdapterSendRoutesAsAdminNotification() throws Exception {
        ...
        try (client) {
            ...
            var adapter = new EventoServerV2Adapter(client, "bundle-A", "inst-A1", 100L);
