		...
		try
		{
			...
			client.newCall(request).enqueue(new Callback() {
				...
				public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
					...
					completableFuture.complete((R) payloadMapper.readValue(Objects.requireNonNull(response.body()).bytes(), Object.class));
					else
						completableFuture.completeExceptionally(new Exception(Objects.requireNonNull(response.body()).string()));
				}
			});
